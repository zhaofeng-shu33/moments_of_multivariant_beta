...
$\E_{\alpha, \beta}[X^m Y^r Z^{2t}] = \sum_{i=0}^r \E_{\alpha, \beta}
[A^{r-i}X^{m-r}Z^{2(t+i)}]$. Then using Equation \ref{eq:Aexp} recursively
we have 
\begin{align*}
\E_{\alpha, \beta}
[A^{r-i}X^{m-r}Z^{2(t+i)}] =&
...
\end{proof}
\section{Conclusion}
We derived the formula of high order moment for multivariate Beta distribution
in second-order matrix case. This result is helpful when corresponding distribution
model is used in statistics.
\section*{References}
