...
\title{2 by 2 matrix extension of Beta distribution and its higher moments}
...
Moments of probability distribution is useful in statistics. Given the moment sequence, the probability distribution is unique under some mind conditions.
To prove the convergence of random variables,  we can prove the convergence of its moment sequences.
...
We are interested in how $\E[S_{12}^{2s}S_{11}^t]$ decreases as $k\to \infty$ ($n \to \infty$ too, since $n > k$).
...
\section{Conclusion}
We have derived the formula of higher order moments for multivariate Beta distribution
