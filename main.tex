...
where $f$ is a monomial about the positive-definite matrix $W$.
...
\begin{theorem}\label{thm:Xm}
	...
	follows Beta distribution $\Beta(\alpha, \beta)$.
\end{theorem}
\begin{proof}
...
when $m<r$ we can exchange $m$ with $r$ and then use Corollary \ref{cor:mr}.
\begin{proof}
...
\end{proof}
\section{Case Study}
In this section, we will give a natural example which illustrates how our result can be used.
...
That is, $\E[S_{11}^mS_{12}^{2t}]$ decreases in the order of $n^{-t}$.
